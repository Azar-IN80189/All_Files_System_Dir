...
public class Reusableclass 
{
	...
	public WebElement explicitWDWaitvisibility(WebDriver driver,WebElement element,int seconds)
	{
		WebDriverWait wdwait = new WebDriverWait(driver, seconds);
		WebElement elements= wdwait.until(ExpectedConditions.visibilityOf(element));
		//wdwait.until(ExpectedConditions.attributeContains(element, attribute, value));	
		return elements;
	}
...
//	}
	...
	// Loading loop code
	public void WaitforLoadingIcontoDisappear(List<WebElement> element) throws InterruptedException {
int count =0;
while (element.size()!=0 && count<10) {
	Thread.sleep(1000);
	count++;
}
	}
