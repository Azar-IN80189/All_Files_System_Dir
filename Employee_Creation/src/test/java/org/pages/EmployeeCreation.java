...
public class EmployeeCreation extends org.base.Reusableclass {
	PageObjectModel p;
	
	static PageObjectModel s;
	@When("User need to enter in to Master and Employee basic details section with {string} and {int}")
	public void user_need_to_enter_in_to_Master_and_Employee_basic_details_section_with_and(String SheetName, Integer RowNumber) throws IOException, InvalidFormatException, InterruptedException, AWTException {
		...
		String SelectShiftEtmsside = testData.get(RowNumber).get("Shift Name ");
	...
	public void user_Create_a_Employee_by_filling_the_employee_form_and(String SheetName, Integer RowNumber) throws IOException, InvalidFormatException, AWTException, InterruptedException {
		ExcelReader reader=new ExcelReader();
		p = new PageObjectModel();
		...
		String empcode = testData.get(RowNumber).get("EmployeeCode");
		p.EmployeeCode.sendKeys(empcode);
		implicitwait();
		p.EmployeeName.clear();
		String empname = testData.get(RowNumber).get("EmployeeName");
		p.EmployeeName.sendKeys(empname);
		implicitwait();
		String gender = testData.get(RowNumber).get("GENDER");
		toselectbyvalue(p.Genderdrop,gender);
		implicitwait();
		String marital = testData.get(RowNumber).get("MARITAL STATUS");
		toselectbyvalue(p.Maritialstatusdrop,marital);
		implicitwait();
		p.EMAIL.clear();
		String empemail = testData.get(RowNumber).get("Employee Email");
		p.EMAIL.sendKeys(empemail);
		implicitwait();
		p.Mobilenumber.clear();
		String empmobile = testData.get(RowNumber).get("Mobile Number");
		p.Mobilenumber.sendKeys(empmobile);
		implicitwait();
		String joiningdate = testData.get(RowNumber).get("Join Date");
		p.JoinDate.sendKeys(joiningdate);
		toenter();
		implicitwait();
		String empprobationmonth = testData.get(RowNumber).get("Probation");
		String fingerprintid = testData.get(RowNumber).get("FaceReaderNumber");
		String hoursworked = testData.get(RowNumber).get("Hours Worked");
		implicitwait();
		...
		String department = testData.get(RowNumber).get("BasicDepartmentname");
		toselectbyvisibletext(p.Department,department);
		Thread.sleep(2000);
		String Section = testData.get(RowNumber).get("BasicSectionname");
		toselectbyvisibletext(p.Section, Section);
		String Designation = testData.get(RowNumber).get("BasicDesignationname");
		toselectbyvisibletext(p.Designation,Designation);
		String ExtraClassification = testData.get(RowNumber).get("BasicDesignationgroupname");
		...
		String Category = testData.get(RowNumber).get("BasicCategoryname");
	...
	public void user_have_to_Reset_and(String SheetName, Integer RowNumber) throws IOException, InvalidFormatException, InterruptedException, AWTException {
		ExcelReader reader=new ExcelReader();
		p = new PageObjectModel();
		...
		List<Map<String,String>> testData=reader.getData("src/test/resources/Excel/Automation.xlsx", SheetName);
		String SearchMailinator = testData.get(RowNumber).get("SearchMail");
		...
		String Newpasswordreset = testData.get(RowNumber).get("Reset Password");
		p.Newpasswordreset.sendKeys(Newpasswordreset);
		Explicitwait(p.ReNewpasswordreset);
		toClick(p.ReNewpasswordreset);
		String ReNewpasswordreset = testData.get(RowNumber).get("Reset Password");
		...
		toenter();
		tohandlewindows();
		String Username = testData.get(RowNumber).get("Employee Email");
		p.Username.sendKeys(Username);
		Thread.sleep(500);
		String password = testData.get(RowNumber).get("Reset Password");
